...
with st.sidebar:
    ...
    # Workout options based on discipline
    if discipline == "Run":
    ...
    # --- DYNAMIC INPUTS (ONLY ONE SECTION ALLOWED) ---
    if discipline == "Bike":
        ...
        # We use unique keys here just to be 100% safe from duplicate errors
        ...
        work_for_calc = st.number_input("Avg Speed", min_value=0.0, value=100.0, key="swim_speed")
    ...
    if st.button("Save to Google Sheets"):
        ...
        try:
            ...
            st.success(f"Saved {discipline} session!")
            st.balloons()
            st.rerun()
